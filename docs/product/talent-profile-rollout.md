...
El trabajador puede cargar o reemplazar un PDF de hasta 5 MB y una foto JPG, PNG o WebP de hasta 3 MB. PostgreSQL conserva solo metadatos y una clave; los archivos se guardan fuera de los recursos estáticos y siempre exigen una sesión válida. En desarrollo y piloto se usa un volumen privado local configurable con `DOCUMENT_STORAGE_ROOT`; antes de producción pública se debe sustituir por un proveedor de objetos, añadir análisis antimalware, transformación segura de imágenes, retención y eliminación auditada.

**La foto** se sirve únicamente al propio trabajador (`GET /api/workers/me/photo`). **El CV ya no** (desde `CN-20260921-005`): además del trabajador (`GET /api/workers/me/cv/download`), la empresa dueña de un turno puede abrirlo desde la lista de postulantes de ese turno con `GET /api/business/shifts/:id/applications/:applicationId/cv`, bajo esta política —implementada una sola vez en `apps/api/src/modules/talent/cv_access.ts` y compartida con el indicador `worker.hasCv` de la lista—:

- Hace falta una postulación **vigente** (`PENDING` o `ACCEPTED`) a un turno **de esa empresa**: postularse es el acto explícito con que el trabajador se presenta a esa empresa, que ya recibe nombre, correo y DNI en esa misma lista. `REJECTED`, `WITHDRAWN` y `CANCELLED` retiran el acceso.
- Y hace falta que el perfil exista y esté **visible** (`isVisible: true`). Oculto o inexistente equivale a no consentido, y eso incluye el CV.
- La regla se evalúa en cada petición, así que ocultar el perfil o rechazar la postulación retira el acceso al instante (verificado contra PostgreSQL).
- **El directorio sigue sin exponer el CV**: la afirmación de la sección "Búsqueda empresarial" (la tarjeta no trae CV ni foto) sigue vigente y está cubierta por una prueba. No hay acceso por invitación ni sin postulación.
- El panel web solo dibuja "Ver CV" cuando la API responde `worker.hasCv: true`; no hay ninguna regla de privacidad en el front.

Pendientes específicos de este acceso, asumidos para el piloto: **consentimiento grueso** (`isVisible` gobierna a la vez el directorio y el CV; no existe una bandera propia `isCvVisible`, así que quien oculta su perfil tampoco puede compartir el CV con la empresa a la que se postula), **sin bitácora de accesos** (no se registra quién abrió qué CV), **sin caducidad** mientras la postulación siga vigente, **sin límite de tasa** en esa ruta y **PDF no verificado** (solo se comprueba la firma `%PDF-` al cargarlo; no hay antimalware ni comprobación de estructura). El copy del panel lo dice tal cual: "PDF subido por el trabajador, solo lectura. Chambeaya no verifica su contenido."
