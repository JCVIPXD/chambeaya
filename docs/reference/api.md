...
- Si `endsAt` **ya pasó**: el turno **no reabre a `PUBLISHED`**; se cierra como `CANCELLED` en el mismo movimiento, para no quedar "fantasma" (visible como activo pero inalcanzable desde `cancelShift`/`deleteShift`/`updateShift`, y sin que nadie pueda postular ni hacer check-in sobre un `endsAt` vencido). Es siempre el caso de un `ABANDONED`, que solo se detecta 60 minutos después de `endsAt`. Cerrar el turno no bloquea resolver la asignación: `resolve` no depende del estado del turno, así que la empresa igual puede decidir si paga, y hacerlo con `outcome: "COMPLETED"` genera igualmente el `Payment` pendiente. **Qué hace `resolve` con el estado del turno (BAJO-5 de `CN-20260918-004`, corregido en `CN-20260920-003` y ajustado en `CN-20260920-005`).** `resolveAssignment` recalcula el estado con el turno que devuelve el ciclo de vida (el que ya pudo cerrarse como `CANCELLED` en esa misma llamada o en una lectura previa), nunca con una lectura anterior a él, así que el resultado ya no depende de si la empresa abrió antes las postulaciones. La regla:
  - `outcome: "COMPLETED"` sobre un turno `CANCELLED` **cerrado por el propio vencimiento** ("sin asignaciones viables"): el turno **pasa a `COMPLETED`** cuando, tras esa confirmación, todos sus cupos quedan confirmados como trabajados (`requiredWorkers` asignaciones efectivas y todas `COMPLETED`; en un turno de un solo cupo es siempre el caso). **Si el recálculo daría `CHECKED_IN`** (turno multi-cupo con algún cupo todavía sin cerrar como trabajado), el turno **se mantiene `CANCELLED`**: reabrirlo a `CHECKED_IN` lo dejaría inalcanzable (`cancelShift`, `updateShift` y `deleteShift` lo rechazan y ningún endpoint lo mueve), el "turno fantasma" que ya se cerró en `CN-20260918-002`. En ambos casos la asignación queda `COMPLETED` y el `Payment` pendiente se registra. Al confirmar el último cupo pendiente el turno pasa a `COMPLETED`. Reabrir deja un `ShiftEvent` `UPDATED` (actor `BUSINESS`) con la transición `CANCELLED → COMPLETED`, además del `COMPLETED` que ya crea `resolve`.
  - **No se reabre** un turno que la empresa canceló con `cancelShift` (deja un `ShiftCancellation` a nivel de turno, con `assignmentId` nulo y `actorRole: BUSINESS`), ni un turno `CANCELLED` cuyo `endsAt` aún no pasó (un cierre por vencimiento exige `endsAt` vencido), ni un multi-cupo con algún cupo sin cerrar (ver arriba). En esos casos la asignación pasa a `COMPLETED` y el `Payment` queda registrado, pero el turno sigue `CANCELLED`.
  - `outcome: "CANCELLED"` (cerrar sin pago) nunca cambia un turno ya `CANCELLED`, y nunca crea `Payment`.
