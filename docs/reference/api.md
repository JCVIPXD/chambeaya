...
- Si `endsAt` **ya pasó**: el turno **no reabre a `PUBLISHED`**; se cierra como `CANCELLED` en el mismo movimiento, para no quedar "fantasma" (visible como activo pero inalcanzable desde `cancelShift`/`deleteShift`/`updateShift`, y sin que nadie pueda postular ni hacer check-in sobre un `endsAt` vencido). Es siempre el caso de un `ABANDONED`, que solo se detecta 60 minutos después de `endsAt`. Cerrar el turno no bloquea resolver la asignación: `resolve` no depende del estado del turno, así que la empresa igual puede decidir si paga, y hacerlo con `outcome: "COMPLETED"` genera igualmente el `Payment` pendiente. **Lo que `resolve` hace con el estado del turno depende de si una lectura previa ya lo cerró:** `resolveAssignment` recalcula con el `status` que leyó al empezar (ver BAJO-5 de `CN-20260918-004`), así que si el turno ya estaba persistido como `CANCELLED` (lo habitual cuando la empresa abrió antes las postulaciones, que es lo que detecta el `NO_SHOW`/`ABANDONED` y cierra el turno) **el turno se queda `CANCELLED` aunque la asignación pase a `COMPLETED` y el pago quede registrado**; solo si `resolve` es el primer contacto con el turno vencido este puede terminar en `COMPLETED`. Verificado contra API y PostgreSQL reales en `apps/web/e2e-real/assignment-resolution.spec.ts` (turno `CANCELLED`, asignación `COMPLETED`, un `Payment` `PENDING`). Un cliente no debe prometer que cobrar reabre o completa el turno.
