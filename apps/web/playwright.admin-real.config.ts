 ...
 * Sin `CUMPLENOW_E2E_REAL_TESTS=true` y `CUMPLENOW_E2E_DATABASE_URL`, el
 * arranque de la API falla con un error explícito antes de tocar ninguna
 * base de datos (ver `apps/api/scripts/e2e-serve.ts`).
 */
const webPort = Number(process.env.CUMPLENOW_E2E_WEB_PORT ?? 3400);
...
export default defineConfig({
  ...
  webServer: [
    {
      command: 'npm exec --workspace=@cumple-now/api -- tsx scripts/e2e-serve.ts',
      cwd: '../..',
      url: `${apiOrigin}/api/health`,
      reuseExistingServer: false,
      timeout: 120_000,
      env: {
        CUMPLENOW_E2E_REAL_TESTS: 'true',
        DATABASE_URL: process.env.CUMPLENOW_E2E_DATABASE_URL ?? '',
        API_PORT: String(apiPort),
      },
    },
    {
      command: `npm run build --workspace=@cumple-now/web && npm run start --workspace=@cumple-now/web -- --hostname 127.0.0.1 --port ${webPort}`,
      cwd: '../..',
      url: baseURL,
      reuseExistingServer: false,
      timeout: 180_000,
